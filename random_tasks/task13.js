...
function createTwoSetsOfEqualSum(n) {
    ...
    let desireElement = (firstSum.reduce((sum,el) => sum + el ,0) - secondSum.reduce((sum,el) => sum + el ,0)) / 2   
    if(firstSum.includes(desireElement)){
        const indexOfdesireElement = firstSum.findIndex(el => el === desireElement)
        secondSum.push(firstSum.splice(indexOfdesireElement,1)[0])
        return [firstSum, secondSum]
    }else{
        desireElement += 1;
        const indexOfdesireElement = firstSum.findIndex(el => el === desireElement)
        secondSum.push(firstSum.splice(indexOfdesireElement,1)[0])
        firstSum.push(secondSum.splice(0,1)[0]);
        return [firstSum, secondSum]
    }
}
