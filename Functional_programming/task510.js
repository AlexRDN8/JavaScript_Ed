...
while
do while
WARNING: Using ES6 features may break these rules because Babel can introduce some for loops 
when transpiling your code. Please, open an issue if it happens.
*/
var multiFilter = function(...fns){
    return function(elem){
        return fns.every(fns => fns(elem));
    }
...
//const res = [1,2,3,4,10,11,12,20,21,22].filter(multiFilter(isEven))//, isGTten)); //[12,20,22]
const res = [1,2,3,4,10,11,12,20,21,22].filter(multiFilter(isEven, isGTten));
console.log(res);