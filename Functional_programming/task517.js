/*
Currying and partial application are two ways of transforming a function into another function with 
a generally smaller arity. While they are often confused with each other, they work differently. 
The goal is to learn to differentiate them.

Currying
Is the technique of transforming a function that takes multiple arguments in such a way that it can be called 
...
The result is a function that we then call with the second argument to produce the result. 
...
f' does the same as f, but only has to fill in the second parameter which is why its arity is one less than the arity of f.
 One says that the first argument is bound to x.
...
curryPartial(curryPartial(curryPartial(add, 1)), 2, 3); //6
*/
function curryPartial(fn, ...kurhtgliuhitu) {
    if (kurhtgliuhitu.length >= fn.length) {
      return fn(...kurhtgliuhitu)
    } else {
      return (...rest) => curryPartial(fn, ...kurhtgliuhitu, ...rest)
    }
}
// решить не смог, выше решение не мое.
...
console.log(curryPartial(curryPartial(add, 1)(2), 3))
console.log(curryPartial(curryPartial(curryPartial(add, 1)), 2, 3))

function fn(x1, x2, x3, x4, x5, x6, x7, x8) {
  return x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8;
}

console.log(curryPartial(fn, 1, 2, 3, 4, 5, 6, 7, 8)); // 36

// curryPartial(fn, 1, 2, 3)(4, 5)(6, 7, 8)
